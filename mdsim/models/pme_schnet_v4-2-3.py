...
@registry.register_model("pme_schnet_v4-2-3")
class PmeSchNetWrap(SchNet):
    ...
    def __init__(
        ...
        self.distance_expansion_erfc = GaussianSmearing(0.0, cutoff, num_gaussians // 2)
    ...
    def _forward(self, data):
        ...
        if self.use_pbc:
            ...
            edge_weight_erfc = torch.special.erfc(edge_weight)
            edge_attr = self.distance_expansion_erfc(edge_weight)
            edge_attr_erfc = self.distance_expansion_erfc(edge_weight_erfc)
            edge_attr = torch.cat([edge_attr, edge_attr_erfc], dim=1)
            ...
            atom_with_mesh_edge_attr = self.distance_expansion_erfc(atom_with_mesh_dist)
            atom_with_mesh_edge_attr_erfc = self.distance_expansion_erfc(
                atom_with_mesh_dist
            )
            atom_with_mesh_edge_attr = torch.cat(
                [atom_with_mesh_edge_attr, atom_with_mesh_edge_attr_erfc], dim=1
            )
            ...
            mesh_energy = scatter(
                mesh_charge, mesh["meshgrid_batch"], dim=0, reduce=self.readout
            )

            h = self.embedding(z)

            for interaction in self.interactions:
                h = (h + interaction(h, edge_index, edge_weight, edge_attr)) * (2**-0.5)
