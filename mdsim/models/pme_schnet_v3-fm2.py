...
@registry.register_model("pme_schnet_v3-fm2")
class PmeSchNetWrap(SchNet):
    ...
    def __init__(
        ...
        self.atom_to_mesh_interaction = InteractionBlock(
            hidden_channels, num_gaussians, num_filters, cutoff
        )

        self.mesh_to_atom_interaction = InteractionBlock(
            hidden_channels, num_gaussians, num_filters, cutoff
        )
    ...
    def _forward(self, data):
        ...
        if self.use_pbc:
            ...
            h_with_mesh = self.atom_to_mesh_interaction(
                h_with_mesh,
            ...
            h_with_mesh = torch.cat([h, mesh_feat], dim=0)
            h_with_mesh = self.mesh_to_atom_interaction(
                h_with_mesh,
