...
@registry.register_model("pme_schnet_v6")
class PmeSchNetWrap(SchNet):
    ...
    def __init__(
        ...
        self.mesh_interactions = ModuleList()
        for _ in range(num_interactions):
            self.mesh_interactions.append(
                InteractionBlock(mesh_channel, num_gaussians, num_filters, cutoff)
            )

        self.mesh_interactions2 = ModuleList()
        for _ in range(num_interactions):
            self.mesh_interactions2.append(
                InteractionBlock(mesh_channel, num_gaussians, num_filters, cutoff)
            )
    ...
    def _forward(self, data):
        ...
        if self.use_pbc:
            ...
            for i in range(self.num_interactions):
                ...
                h_with_mesh = self.mesh_interactions[i](
                    h_with_mesh,
                ...
                h_with_mesh = torch.cat([mesh_charge, mesh_feat], dim=0)
                h_with_mesh = self.mesh_interactions2[i](
                    h_with_mesh,
